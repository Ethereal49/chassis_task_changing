...
int16_t wheel1_v_set = 0;
int16_t wheel1_err = 0;
int16_t wheel1_err_last = 0;
int16_t wheel1_i = 0;
int16_t wheel1_i_set = 0;
int16_t wheel1_i_can = 0;

int16_t wheel2_v_set = 0;
int16_t wheel2_err = 0;
int16_t wheel2_err_last = 0;
int16_t wheel2_i = 0;
int16_t wheel2_i_set = 0;
int16_t wheel2_i_can = 0;

int16_t wheel3_v_set = 0;
int16_t wheel3_err = 0;
int16_t wheel3_err_last = 0;
int16_t wheel3_i = 0;
int16_t wheel3_i_set = 0;
int16_t wheel3_i_can = 0;

int16_t wheel4_v_set = 0;
int16_t wheel4_err = 0;
int16_t wheel4_err_last = 0;
int16_t wheel4_i = 0;
int16_t wheel4_i_set = 0;
int16_t wheel4_i_can = 0;

int16_t vx = 0;
int16_t vy = 0;
int16_t wz = 0;

int16_t speed_buf = 2000;
static void m3508_can_open()
{
  if (wheel1_i_set - wheel1_i_can < speed_buf && wheel1_i_set - wheel1_i_can > -speed_buf)
  {
    wheel1_i_can = wheel1_i_set;
  }
  if (wheel1_i_set - wheel1_i_can > speed_buf)
  {
    wheel1_i_can += speed_buf;
  }
  if (wheel1_i_set - wheel1_i_can < -speed_buf)
  {
    wheel1_i_can -= speed_buf;
  ...
  if (wheel2_i_set - wheel2_i_can < speed_buf && wheel2_i_set - wheel2_i_can > -speed_buf)
  {
    wheel2_i_can = wheel2_i_set;
  }
  if (wheel2_i_set - wheel2_i_can > speed_buf)
  {
    wheel2_i_can += speed_buf;
  }
  if (wheel2_i_set - wheel2_i_can < -speed_buf)
  {
    wheel2_i_can -= speed_buf;
  ...
  if (wheel3_i_set - wheel3_i_can < speed_buf && wheel3_i_set - wheel3_i_can > -speed_buf)
  {
    wheel3_i_can = wheel3_i_set;
  }
  if (wheel3_i_set - wheel3_i_can > speed_buf)
  {
    wheel3_i_can += speed_buf;
  }
  if (wheel3_i_set - wheel3_i_can < -speed_buf)
  {
    wheel3_i_can -= speed_buf;
  ...
  if (wheel4_i_set - wheel4_i_can < speed_buf && wheel4_i_set - wheel4_i_can > -speed_buf)
  {
    wheel4_i_can = wheel4_i_set;
  }
  if (wheel4_i_set - wheel4_i_can > speed_buf)
  {
    wheel4_i_can += speed_buf;
  }
  if (wheel4_i_set - wheel4_i_can < -speed_buf)
  {
    wheel4_i_can -= speed_buf;
  ...
  chassis_move.motor_chassis[0].give_current = wheel1_i_can;
  chassis_move.motor_chassis[1].give_current = wheel2_i_can;
  chassis_move.motor_chassis[2].give_current = wheel3_i_can;
  chassis_move.motor_chassis[3].give_current = wheel4_i_can;
}

static void chassis_v_to_mecanum_wheel_speed(void)
{
  wheel1_v_set = -vx - vy + wz;
  wheel2_v_set = vx - vy + wz;
  wheel3_v_set = vx + vy + wz;
  wheel4_v_set = -vx + vy + wz;

  wheel1_err = wheel1_v_set - chassis_move.motor_chassis[0].chassis_motor_measure->speed_rpm;
  wheel1_i += wheel1_err;

  wheel2_err = wheel2_v_set - chassis_move.motor_chassis[1].chassis_motor_measure->speed_rpm;
  wheel2_i += wheel2_err;

  wheel3_err = wheel3_v_set - chassis_move.motor_chassis[2].chassis_motor_measure->speed_rpm;
  wheel3_i += wheel3_err;

  wheel4_err = wheel4_v_set - chassis_move.motor_chassis[3].chassis_motor_measure->speed_rpm;
  wheel4_i += wheel4_err;

  wheel1_i_set = 0.05 * (0.3 * wheel1_err + 1 * wheel1_i + 0.05 * (wheel1_err - wheel1_err_last));
  wheel2_i_set = 0.05 * (0.3 * wheel2_err + 1 * wheel2_i + 0.05 * (wheel2_err - wheel2_err_last));
  wheel3_i_set = 0.05 * (0.3 * wheel3_err + 1 * wheel3_i + 0.05 * (wheel3_err - wheel3_err_last));
  wheel4_i_set = 0.05 * (0.3 * wheel4_err + 1 * wheel4_i + 0.05 * (wheel4_err - wheel4_err_last));

  wheel1_err_last = wheel1_err;
  wheel2_err_last = wheel2_err;
...
static void chassis_control_test(void)
{
...
void chassis_task(void const *pvParameters)
{
  ...
  // 判断底盘电机是否都在线
  while (0) // toe_is_error(CHASSIS_MOTOR1_TOE) || toe_is_error(CHASSIS_MOTOR2_TOE) || toe_is_error(CHASSIS_MOTOR3_TOE) || toe_is_error(CHASSIS_MOTOR4_TOE) || toe_is_error(DBUS_TOE))
  {
  ...
  while (1)
  {
    ...
    if (1) //!(toe_is_error(CHASSIS_MOTOR1_TOE) && toe_is_error(CHASSIS_MOTOR2_TOE) && toe_is_error(CHASSIS_MOTOR3_TOE) && toe_is_error(CHASSIS_MOTOR4_TOE)))
    {
...
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
  ...
  // 麦轮运动分解
  if ((chassis_move_control_loop->chassis_RC->key.v & KEY_PRESSED_OFFSET_E))
  {
    chassis_move_control_loop->vx_set = 0;
    chassis_move_control_loop->vy_set = 0;
    chassis_move_control_loop->wz_set = 8;
  }
  if (chassis_move_control_loop->chassis_RC->key.v & KEY_PRESSED_OFFSET_Q)
  {
    chassis_move_control_loop->vx_set = 0;
    chassis_move_control_loop->vy_set = 0;
    chassis_move_control_loop->wz_set = -8;
