...
int16_t gimbal_pitch_set=5000;	
int16_t gimbal_pitch_err = 0;
int16_t gimbal_pitch_err_last = 0;
int16_t gimbal_pitch_i = 0;

int16_t gimbal_yaw_err = 0;
int16_t gimbal_yaw_err_last = 0;
int16_t gimbal_yaw_i = 0;	


uint8_t start = 0;
fp32 gimbal_yaw_set =0;
fp32 gimbal_abs_yaw_err = 0;
fp32 gimbal_abs_yaw_err_last = 0;
fp32 gimbal_abs_yaw_i = 0;	

int16_t gimbal_mouse_yaw_set = 6399;
int16_t gimbal_mouse_yaw_err = 0;
int16_t gimbal_mouse_yaw_err_last = 0;
int16_t gimbal_mouse_yaw_i = 0;	


int16_t gimbal_mouse_pitch_set = 5000;
int16_t gimbal_mouse_pitch_err = 0;
int16_t gimbal_mouse_pitch_err_last = 0;
int16_t gimbal_mouse_pitch_i = 0;	


//编码器范围8191
//鼠标速度正负32768
static void gimbal_mouse_control(){
		...
		if(yaw_can_set_current>10000)
		{
				yaw_can_set_current = 10000;
		}
		if(yaw_can_set_current<-10000)
		{
			yaw_can_set_current = -10000;
		}
		if(pitch_can_set_current>10000)
		{
				pitch_can_set_current = 10000;
		}
		if(pitch_can_set_current<-10000)
		{
			pitch_can_set_current = -10000;
...
static void gimbal_yaw_control(void)
{
		//步兵yaw轴中值为3214
		//英雄中值6339
...
{//pitch 角度限制 为4199~5514 最大电流为10000
...
void gimbal_task(void const *pvParameters)
{
    ...
    while (1)
    {
       ...
       // gimbal_control_loop(&gimbal_control);                //云台控制PID计算
        ...
        if (1)//(toe_is_error(YAW_GIMBAL_MOTOR_TOE) && toe_is_error(PITCH_GIMBAL_MOTOR_TOE) && toe_is_error(TRIGGER_MOTOR_TOE)))
        {
...
{
    gimbal_control.gimbal_yaw_motor.offset_ecd = 3214;
    gimbal_control.gimbal_yaw_motor.max_relative_angle =  -0.006670;//max_yaw;
    gimbal_control.gimbal_yaw_motor.min_relative_angle =  -0.008670;//min_yaw;

    gimbal_control.gimbal_pitch_motor.offset_ecd = 2981;
    gimbal_control.gimbal_pitch_motor.max_relative_angle = 0.490107;
    gimbal_control.gimbal_pitch_motor.min_relative_angle = -0.2;//-0.465534;
}
